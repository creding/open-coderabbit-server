...
describe('FileValidator', () => {
  ...
  it('should throw an error if too many files are provided', () => {
    const files = Array(6)
      .fill(0)
      .map((_, i) => createFile(`file${i}.ts`, 'content'));
    expect(() => validator.validateFiles(files)).toThrow(
      new FileValidationError(
        'Too many files: 6 exceeds limit of 5',
        'TOO_MANY_FILES'
      )
    );
  });
  ...
  it('should throw an error if total file size is exceeded', () => {
    ...
    expect(() => validator.validateFiles(files)).toThrow(
      new FileValidationError(
        'Total file size 1.95 KB exceeds limit of 1.95 KB',
        'TOTAL_SIZE_EXCEEDED'
      )
    );
  });
  ...
  it('should throw an error if a single file is too large', () => {
    ...
    expect(() => validator.validateFiles(files)).toThrow(
      new FileValidationError(
        "File 'large.ts' size 1001 Bytes exceeds limit of 1000 Bytes",
        'FILE_TOO_LARGE'
      )
    );
  });
  ...
  it('should throw an error for unsupported file extensions', () => {
    ...
    expect(() => validator.validateFiles(files)).toThrow(
      new FileValidationError(
        "File 'image.jpg' has unsupported extension '.jpg'. Allowed: .ts, .js, .md",
  ...
  it('should throw an error for invalid filenames', () => {
    ...
    expect(() => validator.validateFiles(files)).toThrow(
      new FileValidationError(
        "File 'in<valid>.ts' has invalid filename",
        'INVALID_FILENAME'
      )
    );
  });
  ...
  it('should throw an error for binary content', () => {
    ...
    expect(() => validator.validateFiles(files)).toThrow(
      new FileValidationError(
        "File 'binary.ts' appears to contain binary content",
        'BINARY_CONTENT'
      )
    );
  });
  ...
  it('should throw an error for lines that are too long', () => {
    ...
    expect(() => validator.validateFiles(files)).toThrow(
      new FileValidationError(
        'Total file size 9.77 KB exceeds limit of 1.95 KB',
        'TOTAL_SIZE_EXCEEDED'
      )
    );
  });
