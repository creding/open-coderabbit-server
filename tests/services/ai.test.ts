...
describe('UnifiedAiProvider', () => {
  ...
  it('should generate a review title', async () => {
    ...
    expect(ai.generateObject).toHaveBeenCalledWith(
      expect.objectContaining({ schema: reviewTitleSchema })
    );
  });
  ...
  it('should generate a review summary', async () => {
    ...
    expect(ai.generateObject).toHaveBeenCalledWith(
      expect.objectContaining({ schema: reviewSummarySchema })
    );
  });
  ...
  it('should generate a PR objective', async () => {
    ...
    expect(ai.generateObject).toHaveBeenCalledWith(
      expect.objectContaining({ schema: prObjectiveSchema })
    );
  });
  ...
  it('should generate a walkthrough', async () => {
    ...
    expect(ai.generateObject).toHaveBeenCalledWith(
      expect.objectContaining({ schema: walkThroughSchema })
    );
  });
