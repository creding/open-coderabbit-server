...
export class FileValidator {
  ...
  constructor(config?: Partial<ValidationConfig>) {
    this.config = {
      ...
      allowedExtensions: [
        ...
        '.editorconfig', '.dockerignore', '.htaccess'
      ],
      ...config,
    };
  }
  ...
  private validateTotalSize(files: File[]): void {
    ...
    if (totalSize > this.config.maxTotalSize) {
      throw new FileValidationError(
        `Total file size ${this.formatBytes(totalSize)} exceeds limit of ${this.formatBytes(this.config.maxTotalSize)}`,
        'TOTAL_SIZE_EXCEEDED'
      );
    }
  }
  ...
  private validateSingleFile(file: File): void {
    ...
    if (file.fileContent.length > this.config.maxFileSize) {
      throw new FileValidationError(
        `File '${file.filename}' size ${this.formatBytes(file.fileContent.length)} exceeds limit of ${this.formatBytes(this.config.maxFileSize)}`,
        'FILE_TOO_LARGE'
      );
    }
    ...
    if (!this.config.allowedExtensions.includes(extension)) {
      throw new FileValidationError(
        `File '${file.filename}' has unsupported extension '${extension}'. Allowed: ${this.config.allowedExtensions.join(', ')}`,
  ...
  private validateFileContent(file: File): void {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].length > maxLineLength) {
        throw new FileValidationError(
          `File '${file.filename}' line ${i + 1} is too long (${lines[i].length} chars). This might be minified code.`,
          'LINE_TOO_LONG'
        );
      }
    }
  ...
  private getFileExtension(filename: string): string {
    ...
    if (specialFiles.includes(baseFilename) || baseFilename.startsWith('.env.')) {
